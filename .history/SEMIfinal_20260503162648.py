...
def run_priority_np(processes, higher_is_better):
    ...
    done, gantt, time = reset(processes), [], [], 0 #initialize 
...
class MainApp(tk.Tk):
    ...
    def buildUI(self):
       # HEADER CONTAINER
        ...
        # MAIN PANNEL
        mainPannel = tk.Frame(self, bg=BG)
        mainPannel.pack(fill="both", expand=True, padx=24, pady=16)
        mainPannel.columnconfigure(0, weight=0, minsize=320)
        mainPannel.columnconfigure(1, weight=1)
        mainPannel.rowconfigure(0, weight=1)

        self._build_left(mainPannel)
        self._build_right(mainPannel)
