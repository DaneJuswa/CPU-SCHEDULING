...
def run_sjf(processes):
    procs, done, gantt, time = reset(processes), [], [], 0 # reset data, initialize lists and time
    remaining = procs[:]
    while remaining:   # loop until all process are done
        ...
        remaining.remove(p)
        p["start"] = time
        gantt.append({"pid": p["pid"], "start": time, "end": time + p["bt"]}) # add to the gannt chart array
