...
class CPUSchedulerApp(tk.Tk):
    ...
    def _build_left(self, parent):
        ...
        self.plus = tk.PhotoImage(file="plus.png").subsample(2, 2)
        
        self._flat_btn(btn_row, "+ Add Process", self._add_process_row,
                    fg=ACCENT2, bg=ACCENT, image=self.plus).pack(side="left")

        self._flat_btn(btn_row, "− Remove", self._remove_last_row,
                    fg=ERROR).pack(side="left", padx=(8, 0))

        self._flat_btn(btn_row, "Clear Inputs", self._clear_all_process,
                    fg=ACCENT).pack(side="left", padx=(10, 0))

        # ================= PROCESS TABLE =================
        
        #main card for process / outer div
        proc_card = tk.Frame(left, bg=CARD, bd=0)
        ...
        # # Add / Remove buttons
        # btn_row = tk.Frame(proc_card, bg=CARD, padx=50 )
        # btn_row.pack(fill="x", padx=10, pady=(4,10))
        # self._flat_btn(btn_row, "+ Add Process", self._add_process_row,
        #                fg=ACCENT2).pack(side="left")
        # self._flat_btn(btn_row, "− Remove", self._remove_last_row,
        #                fg=ERROR).pack(side="left", padx=(8,0))
        # self._flat_btn(btn_row, "Clear Inputs", self._clear_all_process, fg=ACCENT).pack(side="left", padx=(10,0))
        ...
        # run_btn.pack(fill="x", pady=(16, 0))

        # ── Stats cards ──
        self._section_label(left, "AVERAGES", pady=(16,4))
    ...
    #button template
    def _flat_btn(self, parent, text, cmd, fg=TEXT, bg=CARD, image=None):
        btn = tk.Button(
            parent,
            text=text,
            image=image,
            compound="center",   # text over image (or use "left")
            fg=fg,
            bg=bg,               # use parameter
            font=("Segoe UI", 9),
            relief="flat",
        ...
        btn.image = image  # ✅ keep reference (VERY important)
        return btn
